...
one_section_title_and_one_prop_test_() ->
  {setup,
   ...
   [
    ...
    %% value has [ char
    ?_assertEqual({ok, #{ <<"title">> => #{ <<"key1">> => <<"va[lue1">> } }},
                  parse(
                    "[title]  \n"
                    "key1=va[lue1\n"
                   )),
    %% value has ] char
    ?_assertEqual({ok, #{ <<"title">> => #{ <<"key1">> => <<"valu]e1">> } }},
                  parse(
                    "[title]  \n"
                    "key1=valu]e1\n"
                   )),
    %% value has [ and ] chars
    ?_assertEqual({ok, #{ <<"title">> => #{ <<"key1">> => <<"va[lu]e1">> } }},
                  parse(
                    "[title]  \n"
                    "key1=va[lu]e1\n"
                   )),
    ...
    %% value has ; char
    ?_assertEqual({ok, #{ <<"title">> => #{ <<"key1">> => <<"value1;continue">> } }},
                  parse(
                    "[title]  \n"
...
two_section_test_() ->
  {setup,
   ...
   fun teardown/1,
   [
    ?_assertEqual({ok, #{<<"title-B">> => #{},<<"title_A">> => #{}}},
                  parse(
                    "[title_A]\n"
                    "[title-B]\n"
                   )),
    ?_assertEqual({ok, #{<<"Title-B">> => #{<<"Key-B1">> => <<"value-B1">>},
                       <<"Title_A">> => #{<<"Key_A1">> => <<"value_A1">>}}},
                  parse(
                    "[Title_A]\n"
                    "Key_A1=value_A1\n"
                    "[Title-B]  \n"
...
binary_two_section_test_() ->
  {setup,
   ...
   fun teardown/1,
   [
    ?_assertEqual({ok, #{<<"titleA">> => #{},<<"titleB">> => #{}}},
                  parse(
                    "[titleA]\n"
                    "[titleB]\n"
                   )),
    ?_assertEqual({ok, #{<<"titleA">> => #{<<"keyA1">> => <<"valueA1">>},
                       <<"titleB">> => #{<<"keyB1">> => <<"valueB1">>}}},
                  parse(
                    <<"[titleA]\n"
                      "keyA1=valueA1\n"
                      "[titleB]  \n"
...
unicode_test_() ->
  {setup,
   ...
   fun teardown/1,
   [
    ?_assertEqual({ok, #{ <<"title">> => #{ <<"key1">> => <<"\xD1\x8E\xD0\xBD\xD0\xB8\xD0\xBA\xD0\xBE\xD0\xB4">>} } },
                  parse(
                    "[title]\n"
